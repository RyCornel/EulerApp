...
//        self.addTarget(self, action: #selector(didTapButton(_ :)), for: .touchUpInside)
    ...
    @objc func didTapButton(_ : UIButton) {
        ...
        let result = collatzFunc()
        self.container.addSubview(Results(result: result.0, sequence: result.1))
        animateIn()
    ...
    func collatzFunc() -> (String, [Int]) {
        ...
        There are \(sequence.count) elements in this array. With the last number being \(sequence[sequence.endIndex - 1]).
